...
class Fuzzer:
    ...
    def fuzz(self, seed: Seed) -> ResultE[list[Seed]]:
        ...
        if PRINT_SEEDS:
            _log = seed[1] | {"confidence": seed[0]}
            print(orjson.dumps(_log, option=orjson.OPT_APPEND_NEWLINE).decode())
    ...
    def runs(
        ...
        for i in range(n_iters):
            match self.run():
                ...
                case Failure(IndexError() as e):  # no more seeds to pick
                    if len(self.scheduler.population) == 0:
                        errs.append(f"{i}: {str(e)}. No more seeds to pick from.")
                        break
                case Failure(e):
